...
document.writeln('<iframe id=meizzDateLayer Author=wayx frameborder=0 src="test.htm" style="position: absolute; width: 144; height: 221; z-index: 9998; display: none"></iframe>');
strFrame='<style>';
...
strFrame+='        <td colspan=3 align=left Author=meizz><span onclick="parent.meizzDayClear()" style="font-size:11px;cursor: hand"';
strFrame+='         Author=meizz title="' + _VersionInfo + '"><u>清空</u></span>&nbsp;<span onclick=parent.closeLayer() style="font-size:11px;cursor: hand"';
...
function document.onkeydown() 
  {
	...
	if (event.keyCode == 8 ) {  //避免回退键回退到test.htm页面
	  	event.returnValue=false;
	}
       }
  }
...
function meizzToday()  //Today Button
  {
    ...
    //meizzSetDay(meizzTheYear,meizzTheMonth);
    if(outObject){
    	outObject.value = getFormatDay(meizzTheYear,meizzTheMonth,today);	
...
function meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  ...
  if (outObject)
  {
    ...
    outObject.value= getFormatDay(yy,mm,n);
    closeLayer(); 
  }
  else {closeLayer(); alert("您所要输出的控件对象并不存在！");}
}

function getFormatDay(yy,mm,n){
	var formatDay = "";
	switch(outtype)	{
		case 1:	
			if(mm<10) mm = "0" + mm;    //调整位数
    		if(n<10) n = "0" + n;		//调整位数
			formatDay = yy + "年" + mm + "月" + n + "日";
    		break;
		default:
			if(mm<10) mm = "0" + mm;    //调整位数
    		if(n<10) n = "0" + n;		//调整位数
